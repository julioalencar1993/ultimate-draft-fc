...
function pickPosition(team, card){ const need=['GK','ZAG','ZAG','LD','LE','VOL','MC','MAT','PE','PD','ATA']; const used=team.players.map(p=>p.assignedPos||p.pos); if(!used.includes(card.pos) && need.includes(card.pos)) return card.pos; const free=need.find(x=>!used.includes(x)); return free||card.pos; }
